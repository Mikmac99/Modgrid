#!/usr/bin/env python3
"""
ModularGrid Price Monitor - Test Script (Modified)
-------------------------------------------------
This script tests the functionality of the ModularGrid Price Monitor.
It includes tests for authentication, web scraping, database operations,
price analysis, and notifications, with optimizations to prevent long-running tests.
"""
...
class TestModularGridMonitor(unittest.TestCase):
    ...
    @patch('modulargrid_scraper.requests.Session')
    def test_login(self, mock_session):
        ...
        # Test login
        success = self.scraper.login("donutheart", "@bIvGv3U@W03hrA2")
        self.assertTrue(success)
        self.assertTrue(self.scraper.authenticated)
        
        # Verify session calls
        mock_session_instance.get.assert_called_with(self.scraper.LOGIN_URL)
    ...
    def test_get_marketplace_listings(self, mock_session):
        ...
        listings = self.scraper.get_marketplace_listings(region="EU")
    ...
    @patch('notification_system.ToastNotifier')
    def test_windows_notification(self, mock_toaster):
        """Test Windows notification functionality."""
        # Mock ToastNotifier
        mock_toaster_instance = MagicMock()
        mock_toaster.return_value = mock_toaster_instance
        ...
        # Test notification
        with patch('notification_system.os.path.join', return_value="icon.ico"):
            result = self.notifier.send_windows_notification(deal)
        
        # Verify notification was sent
        self.assertTrue(result)
        mock_toaster_instance.show_toast.assert_called_once()
    
    @patch('modulargrid_monitor.ModularGridScraper')
    @patch('modulargrid_monitor.ModularGridDatabase')
    def test_run_scan_cycle(self, mock_db, mock_scraper):
        """Test running a complete scan cycle."""
        # Mock components
        mock_scraper_instance = MagicMock()
        mock_scraper_instance.authenticated = True
        
        # Only return a single page of results to avoid long-running test
        mock_scraper_instance.get_marketplace_listings.side_effect = [
            [
                {
                    "listing_id": "L12345",
                    "module_name": "Test Module",
            ...
            []  # Empty second page to end the loop
        ]
        
        mock_scraper.return_value = mock_scraper_instance
        
        mock_db_instance = MagicMock()
        mock_db_instance.find_deals.return_value = [
            {
                "id": 1,
                "listing_id": "L12345",
                "module_id": "12345",
        ...
        monitor = ModularGridMonitor("test_config.json", self.db_path)
        monitor.scraper = mock_scraper_instance
        monitor.db = mock_db_instance
        monitor.analyzer = MagicMock()
        monitor.notifier = MagicMock()
        monitor.notifier.notify_deals.return_value = 1
        
        # Test run_scan_cycle
        num_listings, num_deals, num_notified = monitor.run_scan_cycle()
        ...
        # Verify method calls
        mock_scraper_instance.get_marketplace_listings.assert_called()
        mock_db_instance.find_deals.assert_called()
        monitor.notifier.notify_deals.assert_called()
